...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setIsLoading(true);
    await axios
      .post(`${apiConfig.user.baseUrl}/api/v1/auth/login`, {
        username,
        password,
      })
      .then((response: any) => {
    ...
    <>
      <form method="post" className={`${styles["poppins-semibold"]}`}>
        <div className="form-group first">
          ...
          />
          {errors.Username ? (
            <span className="text-xs text-danger ml-2">{errors.Username}</span>
          ) : null}
        </div>
        <div className="form-group last mb-3">
          ...
          />
          {errors.Password ? (
            <span className="text-xs text-danger ml-2">{errors.Password}</span>
          ) : null}
        </div>
        <Button
          disabled={isLoading}
