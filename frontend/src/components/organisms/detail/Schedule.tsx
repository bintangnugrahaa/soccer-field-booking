...
export default function Schedule({ params }: { params: { uuid: any } }) {
  ...
  const fetchData = async (selectedDate: any) => {
    try {
      ...
      const response = await axios.get(
        `${apiConfig.field.baseUrl}/api/v1/field/schedule/lists/${uuid}`,
        {
          headers: {
    ...
    if (!user) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          await axios
            .post(
              `${apiConfig.order.baseUrl}/api/v1/order`,
              {
  ...
  const toggleCardSelection = (index: any, uuid: any) => {
    ...
    if (card.status !== status.BOOKED) {
      ...
      const hasSelectedCard: boolean = updatedCards.some(
        ...
        <div className="container">
          <div className="row justify-content-center mb-5">
            <div className="col-md-6 text-center">
              <h2 className="section-title mb-3 text-center poppins-bold">
                Jadwal Lapangan
              </h2>
              <p className="poppins-regular">
                Pilih jadwal yang tersedia untuk melakukan reservasi lapangan
                mini soccer sesuai waktu yang Anda inginkan.
              </p>
            </div>
          </div>
          <div className="desktop d-none d-lg-block">
            <div className="row">
              <div className="col-lg-3">
                ...
                <span
                  ...
                  style={{ fontSize: "11px" }}
                >
                  <sup>*</sup>Tap kolom untuk booking
                </span>
              </div>
            </div>
            <br />
            <div className="row">
              {cards?.length > 0 ? (
                cards.map((card: any, index: number) => (
                  <div className="col-lg-2 mt-3" key={index}>
                    <div
                      ...
                      } ${card.status == status.BOOKED ? "booked" : ""}`}
                      style={{ width: "11rem" }}
                        ...
                        <div className="d-flex justify-content-between align-items-center">
                          <i
                            className={`fa-solid fa-xl ${
                              card.isSelected || card.status == status.BOOKED
                                ? "fa-circle-minus"
                                : "fa-circle-plus"
                            } icon`}
                          ></i>
                          <p className="mb-0 poppins-medium">
                            <b>{card.date}</b>
                          </p>
                        </div>
                        <div className="mt-3">
                          ...
                          <h5
                            className="text-center poppins-medium"
                            style={{ marginTop: "-12px" }}
                          >
                            <b>{card.pricePerHour}</b>
                          </h5>
                          <p className="text-center status poppins-bold">
                            {card.status}
                          </p>
                        </div>
                      </div>
                ...
                <div className="col-lg-12">
                  <div
                    className="item text-center poppins-bold"
                    style={{ fontSize: "20px" }}
                  >
            ...
            <div
              className="row mt-4 container-pay"
              style={{ visibility: isPayButtonVisible ? "visible" : "hidden" }}
            >
                        ...
                        <div className="col-4 mt-4" key={index}>
                          <div className="team">
                            <div
                              ...
                              style={{ width: "6rem" }}
                              onClick={() =>
                                toggleCardSelection(index, card.uuid)
                              }
                            >
